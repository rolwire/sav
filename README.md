...
- **Highlight styles**: countries paint their flag; `--style neon` gives the
  glowing-outline look that dims everything outside the border; hatch is the
  fallback.
- **Auto ruler**: long segments get an animated "X miles / Y km" ruler that
  measures the place's real east–west width.
- **Sound effects**: whoosh on each zoom, ding when the label lands, pop on
  photos — synthesized WAVs generated by the pipeline itself (no licensed
  audio needed).
